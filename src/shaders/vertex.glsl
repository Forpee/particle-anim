...
void main()
{
    ...
    // newpos.z+=cnoise(vec4(position,uTime));
    newpos.y-=mod(uTime/10.,3.);
    ...
    vec3 noise2=2.*(position+uTime*vec3(0.,.1,0.))*vec3(3.,1.,1.);
    
    newpos.x+=.4*progress*cnoise(vec4(noise1,uTime));
    newpos.yz+=.4*progress1*(cnoise(vec4(noise2,uTime))+.2);
